...
def select_candidates_in_gts(xy_centers, gt_bboxes, eps=1e-9, roll_out=False):
    ...
    # Calculate the distances between each ground truth box and the top-left, bottom-right corners of each anchor respectively,
    # then compute the minimum value of these distances. This step ensures that the ground truth box is adjacent to the anchor and encloses it.
    if roll_out:
        ...
        # The top-left and bottom-right coordinates of the ground truth box
...
def select_highest_overlaps(mask_pos, overlaps, n_max_boxes):
    ...
    if fg_mask.max() > 1:  
        ...
        fg_mask             = mask_pos.sum(-2)
    # Find which gt each anchor corresponds to
...
class TaskAlignedAssigner(nn.Module):
    ...
    def get_pos_mask(self, pd_scores, pd_bboxes, gt_labels, gt_bboxes, anc_points, mask_gt):
        ...
        # Positive sample anchors must satisfy the following conditions simultaneously:
        # 1.Be located within the ground truth box
        # 2.Be among the top-k positive samples with the highest overlap with the ground truth box
        # 3.Satisfy the mask_gt constraint
        
        # Determine whether the prior point lies within the ground truth box
        # get in_gts mask           b, max_num_obj, 8400
        mask_in_gts             = select_candidates_in_gts(anc_points, gt_bboxes, roll_out=self.roll_out)
        # Determine whether the anchor is among the top-k candidates of the ground truth box
        ...
        # The ground truth box exists and is not a padding box
    ...
    def get_box_metrics(self, pd_scores, pd_bboxes, gt_labels, gt_bboxes):
        if self.roll_out:
            ...
            # [0] indicates which image it refers to
            # b, max_num_obj  
            ind[0] = torch.arange(end=self.bs).view(-1, 1).repeat(1, self.n_max_boxes)  
            # [1] indicates what the real label is
    ...
    def get_targets(self, gt_labels, gt_bboxes, target_gt_idx, fg_mask):
        ...
        # Obtain the indices of gt_labels and gt_bboxes after flattening
        # b, h*w    
        target_gt_idx   = target_gt_idx + batch_ind * self.n_max_boxes
        # Used for reading labels after flattening
        # b, h*w    
        target_labels   = gt_labels.long().flatten()[target_gt_idx]
        # Used for reading boxes after flattening
        # b, h*w, 4 
...
class BboxLoss(nn.Module):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask):
        # Calculate IoU loss
        # weight represents the confidence that the label should have in the loss, 0 is minimum, 1 is maximum
        weight      = torch.masked_select(target_scores.sum(-1), fg_mask).unsqueeze(-1)
        # Calculate the degree of overlap between predicted and ground truth boxes
        iou         = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, CIoU=True)
        # Then 1-overlap degree, multiply by the confidence it should have, sum and then average
...
class Loss_adv:
    ...
    def __call__(self, preds, batch):
        ...
        # Reshape the elements in a batch into a matrix, where the dimensions are defined as follows:
        # Dimension 0: Index of the image in the batch (indicating which image it belongs to)
        # Dimension 1: Category (class) of the object
        # Dimension 2: Coordinates of the bounding box
        ...
        loss[1] *= 0.5  # cls gain
        loss[2] *= 1.5  # dfl gain
        return loss # loss(box, cls, dfl) # * batch_size
