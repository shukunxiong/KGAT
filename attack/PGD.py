...
class PGD(Attacker):
    ...
    def step(self, images, boxes, loss):
        ...
        #--------The following content is applicable to gray images-------------
        mean_eta            = eta.mean(dim=1, keepdim=True)  

        eta_uniform         = mean_eta.expand(-1, 3, -1, -1)  
        images              = torch.clamp(self.ori_images + eta_uniform, min=0, max=255).detach_()
        #--------Modifications for grayscale images end here---------------------
