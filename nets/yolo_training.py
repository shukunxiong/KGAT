...
import numpy as np
import cv2  # For calculating grayscale mean values
...
def compute_grayscale_means(target_bboxes, target_scores, fg_mask, gt_labels, gt_bboxes, mask_gt, Images_adjust, rate, topk=5, threshold=0.1):
    ...
    for i in range(batch_size):
        ...
        # Scale and adjust bounding boxes to ensure boxes do not exceed image boundaries and are integers
        ...
        # Convert target_scores to softmax output
        target_scores_softmax = F.softmax(target_scores_batch, dim=-1)

        # --------Prevent no predictions from being made, mainly for adversarial examples
        if target_scores_softmax.numel() == 0:  # Check if tensor is empty
            continue  # Continue loop

        # 1. Calculate grayscale mean of predicted boxes
        ...
        for class_idx in range(target_scores_softmax.shape[1]):
            # Process each class, select bounding boxes where softmax value is greater than threshold τ
            ...
            # If there are selected bounding boxes, calculate grayscale mean
            if selected_boxes.size(0) > 0:
                ...
                for box in selected_boxes:
                    ...
                    image_width = image.size(1)  # Image width
                    image_height = image.size(0)  # Image height
                    x1, y1, x2, y2 = box.int().tolist()
                    x1 = max(0, min(x1, image_width - 1))
                    y1 = max(0, min(y1, image_height - 1))
                    x2 = max(0, min(x2, image_width - 1))
                    y2 = max(0, min(y2, image_height - 1))
                    region = image[y1:y2, x1:x2]  # Get region
                    if region.size == 0:
                        continue
                ...
                # If no box, grayscale mean is 0
                predicted_class_grayscale_means[class_idx] = 0

        # 2. Calculate grayscale mean of ground truth boxes
        labeled_class_grayscale_means = torch.zeros(target_scores_softmax.shape[1], device=image.device)

        for class_idx in range(target_scores_softmax.shape[1]):  # Process all ground truth labels
            # Only select gt boxes for the corresponding class
        ...
        # Calculate grayscale mean of background
        ...
        # Add background grayscale values to the end of class grayscale values
        ...
        # Save predicted and labeled grayscale means for each batch
    ...
    # Return two grayscale value tensors, which are the grayscale means of predictions and labels
