...
def autopad(k, p=None, d=1):  
    ...
    # Perform automatic padding on the input feature map according to the Same padding principle
    if d > 1:
        # Calculate the actual kernel size
        k = d * (k - 1) + 1 if isinstance(k, int) else [d * (x - 1) + 1 for x in k]
    if p is None:
        # Automatically calculate the padding size
...
class Conv(nn.Module):
    # Standard convolution + batch normalization + activation function
    default_act = SiLU() 
...
class Bottleneck(nn.Module):
    ...
    # c1: number of input channels, c2: number of output channels
...
class C2f(nn.Module):
    ...
    def forward(self, x):
        # Perform a convolution, then split into two parts, each with c channels
        y = list(self.cv1(x).split((self.c, self.c), 1))
        # Retain the result after each residual structure operation, then stack them together for dense residuals
        y.extend(m(y[-1]) for m in self.m)
        return self.cv2(torch.cat(y, 1))
    
class SPPF(nn.Module):
    # SPP structure, max-pooling with 5, 9, 13 max-pooling kernels.
...
class Backbone(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.dark2(x)
        #-----------------------------------------------#
        #   Output of dark3: 256, 80, 80 (a valid feature map)
        #-----------------------------------------------#
        x = self.dark3(x)
        feat1 = x
        #-----------------------------------------------#
        #   Output of dark4: 512, 40, 40 (a valid feature map)
        #-----------------------------------------------#
        x = self.dark4(x)
        feat2 = x
        #-----------------------------------------------#
        #   Output of dark5: 1024 * deep_mul, 20, 20 (a valid feature map)
        #-----------------------------------------------#
        x = self.dark5(x)
        feat3 = x
