...
def add_disturbance(image_tensor, blur_radius=0, salt_prob=0, gaussian_var=0):
    # Convert input image from tensor to PIL image
...
def load_image_and_labels(image_path, annotation_path, input_shape,class_file):
    """
    Load image and corresponding labels, return labels in labels_out format.
    :param image_path: Path to the image
    :param annotation_path: Path to the corresponding VOC format annotation file
    :param input_shape: Input image dimensions (width, height), used for normalization
    ...
    iw, ih = old_image.size  # Get original image width and height
    h, w = input_shape    # Get target image dimensions
    # Get scale factor for target dimensions
    scale = min(w/iw, h/ih)
    # Get the scale ratio that ensures the image is resized without distortion
    nw = int(iw*scale)
    nh = int(ih*scale)
    # Pad the remaining area with gray bars, here we get the size of the gray bars
    dx = (w-nw)//2
    dy = (h-nh)//2
    ...
    for obj in root.findall('object'):
        ...
        left = float(bndbox.find('xmin').text)  # Use float() instead of int()
        top = float(bndbox.find('ymin').text)   # Use float() instead of int()
        right = float(bndbox.find('xmax').text) # Use float() instead of int()
        bottom = float(bndbox.find('ymax').text)# Use float() instead of int()

        # Adjust coordinates
        box = [left *nw/iw + dx, top *nh/ih + dy, right *nw/iw + dx, bottom *nh/ih + dy]

        # Convert floating-point coordinate values to integers (if needed)
        left = int(left)
        top = int(top)
        right = int(right)
        bottom = int(bottom)
        ...
        scale_h = nh/ih
        scale_w = nw/iw
        # Further clip to ensure bounding box validity
        box[0] = np.clip(box[0],0, w)
        box[1] = np.clip(box[1],0, h)
        box[2] = np.clip(box[2],0, w)
        box[3] = np.clip(box[3],0, h)
        # Calculate the center coordinates and dimensions of the ground truth box
        box[2] = box[2] - box[0]  # Width
        box[3] = box[3] - box[1]  # Height
        box[0] = box[0] + box[2] / 2  # Center X
        box[1] = box[1] + box[3] / 2  # Center Y
        # Convert object names to numeric IDs
        if obj_name in class_names:
            class_id = class_names[obj_name]  # Find the corresponding class ID by class name
        else:
            ...
            continue  # Skip this object if the class is not in the class file

        # Store label information
        labels_out.append([0, class_id, *box])  # Format: [label_ID, class, x_center, y_center, width, height]
...
if __name__ == "__main__":
    ...
    #   Since calculating mAP requires obtaining nearly all prediction boxes, the confidence defined above cannot be changed arbitrarily.
