...
from nets.yolo_training import (Loss_Knowledge, ModelEMA, get_lr_scheduler,
                                set_optimizer_lr, weights_init)
from utils.callbacks import EvalCallback, LossHistory
from utils.dataloader import YoloDataset, yolo_dataset_collate
from utils.utils import (download_weights, get_classes, seed_everything,
                         show_config, worker_init_fn)
from utils.utils_fit import fit_one_epoch_Knowledge
from attack import *
from robust import *
...
if __name__ == "__main__":
    #---------------------------------#
    #   This step sets the type of adversarial training, including cls, loc, cwat, mtd.
    #   Among them, eps is the strength of the adversarial perturbation; alpha is the perturbation strength when generating adversarial examples, and subsequent clipping will align this strength with eps.
    #   iter represents the number of iterations for the adversarial attack.
    #-------------------------------#
    adv_type = 'mtd'
    eps = 8/255
    alpha = 8/255
    iters = 2
    #----------------this is the relation of your data----------------#
    reweight_dir = "your_dataset/dataset_relation/your_dataset_relation.pt"
    #-----------------------------------------------------------------#
    
    #---------exponent is used for adjust the strength for knowledge-reliability optimization----------#
    exponent = 1.0
    #---------------------------------------------------------------------------------------------------
    
    #----------n is used to control the num of adversarial samples in a training epoch-----------------#
    n        = 2
    #--------------------------------------------------------------------------------------------------#
    ...
    #---------------------------------------------------------------------#
    classes_path    = 'KGAT_yolo_v8/model_data/your_data.txt'
    #----------------------------------------------------------------------------------------------------------------------------#
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    model_path      = '/root/autodl-tmp/weight/yolov8/flir/best_yolo_flir_clean.pth'
    #------------------------------------------------------#
    ...
    #----------------------#
    yolo_loss = Loss_Knowledge(model,reweight_dir,exponent)
    ...
    if local_rank == 0:
        ...
        log_dir         = os.path.join(save_dir, "loss_FLIR2_reweight_" + str(adv_type))
    ...
    if True:
        ...
        #-------------Start loading adversarial training components--------------#
        ...
        # adv_dict = {'cls': (CLS_ADG, fasterrcnn_loss ), 'loc': (LOC_ADG, fasterrcnn_loss ), 'con': (CON_ADG, fasterrcnn_loss),
        #         'mtd': (MTD, fasterrcnn_loss ), 'cwat': (CWAT, criterion_clsw)}
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            if epoch >= Freeze_Epoch and not UnFreeze_flag and Freeze_Train:
                ...
                if epoch_step == 0 or epoch_step_val == 0:
                    raise ValueError("The data is too small for training")
            ...
            fit_one_epoch_Knowledge(model_train, model, ema, yolo_loss, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, fp16, scaler, save_period, log_dir, adv_generator,local_rank,n = n)
