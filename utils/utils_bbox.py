...
class DecodeBox():
    ...
    def decode_box(self, inputs):
        ...
        # Perform normalization to the range [0, 1]
    ...
    def yolo_correct_boxes(self, box_xy, box_wh, input_shape, image_shape, letterbox_image):
        #-----------------------------------------------------------------------------------------------------------------#
        #  Place the y-axis first to facilitate multiplication with the image's width and height for the prediction boxes
        #-----------------------------------------------------------------------------------------------------------------#
        box_yx = box_xy[..., ::-1]
        box_hw = box_wh[..., ::-1]
        ...
        if letterbox_image:
            #--------------------------------------------------------------------------------------------------------------------------#
            #   The offset calculated here represents the position of the valid image area relative to the top-left corner of the image
    ...
    def non_max_suppression(self, prediction, num_classes, input_shape, image_shape, letterbox_image, conf_thres=0.5, nms_thres=0.4):
        #----------------------------------------------------------------------------------------#
        #   Convert the format of the prediction results to the top-left and bottom-right format.
        ...
        for i, image_pred in enumerate(prediction):
            #----------------------------------------------------------#
            #   Take the max value for the class prediction part.
            #   class_conf  [num_anchors, 1]    Class confidence
            #   class_pred  [num_anchors, 1]    Class label
            #----------------------------------------------------------#
            class_conf, class_pred = torch.max(image_pred[:, 4:4 + num_classes], 1, keepdim=True)

            #----------------------------------------------------------#
            #   Perform the first round of filtering using confidence.
            #----------------------------------------------------------#
            conf_mask = (class_conf[:, 0] >= conf_thres).squeeze()
            
            #----------------------------------------------------------#
            #   Filter the prediction results based on confidence.
            #----------------------------------------------------------#
            ...
            #   The 6 elements are: x1, y1, x2, y2, class_conf, class_pred
            #-------------------------------------------------------------------------#
            detections = torch.cat((image_pred[:, :4], class_conf.float(), class_pred.float()), 1)

            #------------------------------------------#
            #   Get all classes contained in the prediction results
            #------------------------------------------#
            ...
            for c in unique_labels:
                ...
                #-------------------------------------------------------------------------------------#
                #   Using the official built-in non-max suppression is faster!
                #   Filter out the box with the highest score for the same class within a certain area
                #-------------------------------------------------------------------------------------#
                keep = nms(
                    detections_class[:, :4],
                    detections_class[:, 4],
                ...
                #     # Take the one with the highest confidence for this class, then judge step by step. If the overlap is greater than nms_thres, remove it
...
if __name__ == "__main__":
    ...
    #-----------------------------------------------------------------------#
    #   Convert each feature layer of the predicted values to actual values
    #-----------------------------------------------------------------------#
    def get_anchors_and_decode(input, input_shape, anchors, anchors_mask, num_classes):
        ...
        #---------------------------------------------------------------------------------------#
        #   When input is 640x640, input_shape = [640, 640], input_height = 20, input_width = 20
        #   640 / 20 = 32
        ...
        #--------------------------------------------------------------------------#
        #   The size of scaled_anchors at this time is relative to the feature layer
        ...
        #--------------------------------------------------------------------#
        #   Generate grid and anchor box centers (top-left corner of the grid)
        #   batch_size,3,20,20
        #   range(20)
        #   [
        #       [0, 1, 2, 3 ……, 19], 
        ...
        #---------------------------------------------------------------#
        #   Generate the width and height of anchor boxes in grid format
        ...
        #   First, adjust the center of the anchor box by shifting it from the anchor center to the bottom-right
        #   Then adjust the width and height of the anchor box.
